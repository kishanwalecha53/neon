...
pub fn main() -> Result<()> {
    ...
    if std::env::var_os("NEON_CTL_VERBOSE").is_some() {
        tracing::info!("[tomo-id-007] neon/pageserver/ctl/src/draw_timeline_dir.rs: num_images={num_images}");
        tracing::info!("[tomo-id-008] neon/pageserver/ctl/src/draw_timeline_dir.rs: num_deltas={num_deltas}");
    }
