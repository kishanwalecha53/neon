...
impl ChannelPool {
        ...
        <E as TryInto<Endpoint>>::Error: std::error::Error + Send + Sync,
    {
        ...
        let endpoint = endpoint.connect_timeout(Duration::from_secs(5));
...
impl StreamGuard {
    ...
    pub async fn send(
        ...
        if !self.can_reuse {
            tracing::error!("[tomo-id-005] neon/pageserver/client_grpc/src/pool.rs: send called while stream not reusable request_id={req_id}");
            return Err(tonic::Status::internal(format!(
                "stream can't be reused request_id={req_id}"
            )));
        }
            ...
            loop {
                tokio::select! {
                    _ = tokio::time::sleep(interval) => {
                        ...
                        if std::env::var_os("NEON_GRPC_POOL_VERBOSE").is_some() {
                            tracing::debug!("[tomo-id-004] neon/pageserver/client_grpc/src/pool.rs: reaper tick threshold_ms={} interval_ms={}", threshold.as_millis(), interval.as_millis());
                        }
