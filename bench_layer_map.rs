...
fn build_layer_map(filename_dump: PathBuf) -> LayerMap {
    ...
    let filenames = BufReader::new(
        File::open(&filename_dump).expect("neon/pageserver/benches/bench_layer_map.rs: open layer dump fixture"),
    )
    .lines();

    let mut updates = layer_map.batch_update();
    for fname in filenames {
        let fname = fname.expect("neon/pageserver/benches/bench_layer_map.rs: read fixture line");
        let fname = LayerName::from_str(&fname)
            .expect("neon/pageserver/benches/bench_layer_map.rs: parse layer name from fixture line");
    ...
    if std::env::var_os("NEON_BENCH_VERBOSE").is_some() {
        tracing::info!("[tomo-id-001] neon/pageserver/benches/bench_layer_map.rs: built layer map lsn_bounds min={min_lsn} max={max_lsn}");
    }
