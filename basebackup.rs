    ...
    tokio::spawn({
        ...
        async move {
            ...
            loop {
                ...
                info!(
                    "[tomo-id-003] neon/pageserver/pagebench/src/cmd/basebackup.rs: RPS: {:.0}",
...
async fn run_worker(
    ...
    while let Some(Work { lsn }) = work.recv().await {
        ...
        let size = futures::io::copy(stream.compat(), &mut tokio::io::sink().compat_write())
            .await
            .unwrap();
        info!("[tomo-id-004] neon/pageserver/pagebench/src/cmd/basebackup.rs: basebackup size is {size} bytes");
        let elapsed = start.elapsed();
        live_stats.inc();
        STATS.with(|stats| {
