...
async fn search_layers(
    ...
    let tenant_id = TenantId::from_str(tenant_id).context("neon/pageserver/ctl/src/index_part.rs: parse tenant_id")?;
    let tenant_shard_id = TenantShardId::unsharded(tenant_id);
    let timeline_id = TimelineId::from_str(timeline_id).context("neon/pageserver/ctl/src/index_part.rs: parse timeline_id")?;
    let index_json = {
        let bytes = tokio::fs::read(path).await.context("neon/pageserver/ctl/src/index_part.rs: read index_part")?;
        IndexPart::from_json_bytes(&bytes).context("neon/pageserver/ctl/src/index_part.rs: parse index_part json")?
    };
